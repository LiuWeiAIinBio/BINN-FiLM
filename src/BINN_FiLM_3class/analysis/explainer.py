...
class BINNExplainer:
    ...
    def explain_single(
        ...
        """
        back_inputs, _ = self._gather_all_from_dataloader(back_dataloaders)
        test_inputs, _ = self._gather_all_from_dataloader(test_dataloaders)
        back_inputs = back_inputs.to(config.device)
        test_inputs = test_inputs.to(config.device)

        shap_dict = self._explain_layers(background_data=back_inputs, test_data=test_inputs)
