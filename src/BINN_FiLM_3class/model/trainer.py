            ...
            for epoch in range(num_epochs):
                ...
                with torch.no_grad():
                    for i, data in enumerate(val_loader):
                        ...
                        prob_cat = torch.cat([prob0, prob1, prob2], dim=1)
                        ...
                        # index_1暂存预测结果0/1/2
                        # 算法1
                        # index_1 = index.clone()  # 利用index的形状
                        # mask_0 = (index == 0) | (index == 2)
                        # mask_1 = ((index == 1) | (index == 3)) & (torch.argmax(prob2, dim=1) == 0)
                        # mask_2 = ((index == 1) | (index == 3)) & (torch.argmax(prob2, dim=1) == 1)
                        # index_1[mask_0] = 0
                        # index_1[mask_1] = 1
                        # index_1[mask_2] = 2
                        # # 算法2
                        # index_1 = index.clone()  # 利用index的形状
                        # mask_0 = (prob_cat[:, 0] > prob_cat[:, 1]) & (prob_cat[:, 2] > prob_cat[:, 3])
                        # mask_1 = (((prob_cat[:, 0] < prob_cat[:, 1]) & (prob_cat[:, 2] < prob_cat[:, 3])) | ((prob_cat[:, 0] < prob_cat[:, 1]) & (prob_cat[:, 2] > prob_cat[:, 3])) | ((prob_cat[:, 0] > prob_cat[:, 1]) & (prob_cat[:, 2] < prob_cat[:, 3]))) & (torch.argmax(prob2, dim=1) == 0)
                        # mask_2 = (((prob_cat[:, 0] < prob_cat[:, 1]) & (prob_cat[:, 2] < prob_cat[:, 3])) | ((prob_cat[:, 0] < prob_cat[:, 1]) & (prob_cat[:, 2] > prob_cat[:, 3])) | ((prob_cat[:, 0] > prob_cat[:, 1]) & (prob_cat[:, 2] < prob_cat[:, 3]))) & (torch.argmax(prob2, dim=1) == 1)
                        # index_1[mask_0] = 0
                        # index_1[mask_1] = 1
                        # index_1[mask_2] = 2
                        # 算法3
                        ...
                        mask_1 = (torch.argmax(prob0, dim=1) == 1) & (torch.argmax(prob2, dim=1) == 0)
                        mask_2 = (torch.argmax(prob1, dim=1) == 1) & (torch.argmax(prob2, dim=1) == 1)
                        index_1[mask_0] = 0
                        index_1[mask_1] = 1
                        index_1[mask_2] = 2

                        # 选出outputs计算loss
                        mask_013 = ((prob_cat[:, 0] > prob_cat[:, 1]) & (prob_cat[:, 2] > prob_cat[:, 3])) & (prob_cat[:, 0] > prob_cat[:, 2])
                        mask_213 = ((prob_cat[:, 0] > prob_cat[:, 1]) & (prob_cat[:, 2] > prob_cat[:, 3])) & (prob_cat[:, 0] < prob_cat[:, 2])
                        mask_045 = (((prob_cat[:, 0] < prob_cat[:, 1]) & (prob_cat[:, 2] < prob_cat[:, 3])) | ((prob_cat[:, 0] < prob_cat[:, 1]) & (prob_cat[:, 2] > prob_cat[:, 3])) | ((prob_cat[:, 0] > prob_cat[:, 1]) & (prob_cat[:, 2] < prob_cat[:, 3]))) & (prob_cat[:, 0] < prob_cat[:, 2])
                        mask_245 = (((prob_cat[:, 0] < prob_cat[:, 1]) & (prob_cat[:, 2] < prob_cat[:, 3])) | ((prob_cat[:, 0] < prob_cat[:, 1]) & (prob_cat[:, 2] > prob_cat[:, 3])) | ((prob_cat[:, 0] > prob_cat[:, 1]) & (prob_cat[:, 2] < prob_cat[:, 3]))) & (prob_cat[:, 0] > prob_cat[:, 2])

                        outputs_cat = torch.cat([outputs_0, outputs_1, outputs_2], dim=1)
                        outputs = torch.zeros(outputs_cat.shape[0], 3)
                        outputs[mask_013] = outputs_cat[mask_013][:, [0, 1, 3]]
                        outputs[mask_213] = outputs_cat[mask_213][:, [2, 1, 3]]
                        outputs[mask_045] = outputs_cat[mask_045][:, [0, 4, 5]]
                        outputs[mask_245] = outputs_cat[mask_245][:, [2, 4, 5]]

                        # 对齐outputs创建prob
                        prob = torch.zeros(prob_cat.shape[0], 3, device=self.device)
                        prob[mask_013] = prob_cat[mask_013][:, [0, 1, 3]]
                        prob[mask_213] = prob_cat[mask_213][:, [2, 1, 3]]
                        prob[mask_045] = prob_cat[mask_045][:, [0, 4, 5]]
                        prob[mask_245] = prob_cat[mask_245][:, [2, 4, 5]]
                        print(epoch, "\n", prob_cat, "\n", index_0, "\n", index_1, "\n", prob)
                    ...
                    #     if epoch % 499 == 0:
                    #         final_out_logits_val.append(prob)
                    #         logits_labels_val.append(index_0)
                    #
