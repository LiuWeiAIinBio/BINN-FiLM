...
class BINN(nn.Module):
    ...
    #     return x
        ...
        for dim in modulation_dims:
            self.modulation_heads.append(nn.Sequential(nn.Linear(4, 128),
                                                       nn.Linear(128, 128),
                                                       nn.Linear(128, 2 * dim)))  # 每个头输出 2*dim（gamma和beta各占dim维度）
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> list[Any]:
        ...
        for head in self.modulation_heads:
            # embed task_ID
            t_encoded = torch.zeros(*t.shape, 4, device=t.device, dtype=t.dtype)
            mask_0 = (t == 0)
            mask_1 = (t == 1)
            mask_2 = (t == 2)
            mask_3 = (t == 3)
            t_encoded[mask_0] = torch.tensor([1, 0, 0, 0], device=t.device, dtype=t.dtype)
            t_encoded[mask_1] = torch.tensor([0, 1, 0, 0], device=t.device, dtype=t.dtype)
            t_encoded[mask_2] = torch.tensor([0, 0, 1, 0], device=t.device, dtype=t.dtype)
            t_encoded[mask_3] = torch.tensor([0, 0, 0, 1], device=t.device, dtype=t.dtype)
